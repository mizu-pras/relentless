...
- In relentless, Sentinel is the code reviewer and has authority over Artisan/Maestro
- Critical findings from Sentinel MUST be addressed immediately — no pushback allowed
- Important findings should be addressed before proceeding — reasoned pushback allowed only with evidence
- Advisory findings are noted — implement if time permits
- Check `.relentless/halt` before implementing review feedback
- Report to Conductor if review feedback requires touching files outside your assigned_files
...
In relentless pursuit flow, Sentinel is the external reviewer of record.
```
BEFORE implementing:
...
Push back when:
- Finding is Important (not Critical) and evidence shows it is technically incorrect
...
**In relentless, Sentinel feedback is triaged by severity: Critical = mandatory now, Important = address before proceeding unless disproven with evidence, Advisory = optional backlog.**
